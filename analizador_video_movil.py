...
def textobn(imagen,texto,offset,scale=1,thickness=2):
    fnt = cv2.FONT_HERSHEY_SIMPLEX
    (cancho,calto), base = cv2.getTextSize(texto,fnt,scale,thickness)
    cv2.rectangle(imagen, (offset[0]-5,offset[1]-5), (offset[0]+cancho+5,offset[1]+calto+5), (0,0,0), -1)
    cv2.putText(imagen,texto,(offset[0],offset[1]+calto),fontFace=fnt,fontScale=scale,color=(255,255,255),thickness=thickness)
...
for n in range(11,13):
    ...
    while(video.isOpened()):
        ...
        if ((nframe>149) & (nframe<tot_frames-200)): # ignoramos los primeros segundos
            ...
            textobn(im_copia,texto_fuera,fuera,0.7,2)
            textobn(im_copia,texto_dentro,dentro,0.7,2)
            textobn(im_copia,texto_medio,medio,1,2)
            ...
            textobn(im_copia,tiempo_inst,arriba,3,2)
